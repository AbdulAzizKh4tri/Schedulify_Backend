...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def csv_upload(self, request):
        ...
        if not isinstance(users_list, list):
            return Response(
                {"detail": "Invalid format, 'users' must be a list"}, status=400
            )
        ...
        return Response(
            {"detail": f"CSV processed. Created: {created}, Updated: {updated}"}
        )
        ...
        if user.role == User.TEACHER:
            try:
                ...
                profile_data = {
                    ...
                    "timetable": TimetableEntrySerializer(
                        timetable_entries, many=True
                    ).data,
                }
